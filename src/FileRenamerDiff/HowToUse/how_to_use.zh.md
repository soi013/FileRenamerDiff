...
1. 目标文件规格
您可以在这里拖放文件，或者从特定目录中 <kbd>檔案搜尋</kbd>。

2. 设置重命名模式
从 <kbd>设置</kbd>中设置删除/替换模式。

3. 确认重命名
执行 <kbd>試執行</kbd>，确认重命名。

4. 执行 "重命名"。
执行 <kbd>保存重命名</kbd>来重命名实际文件。
...
| Regex              | 描述                                                  | 模式    | 输入                      | 输出     |
| ------------------ | ----------------------------------------------------- | ------- | ------------------------- | -------- |
| `[`ch_group`]`     | 匹配*ch_group*中的任何一个字符。                      | [ae]    | gr**a**y, L**A**N**E**    | gry,LN   |
...
| `\`escape          | 将转义字符如`. `或`*`等识别为正常字符。               | \\\.    | d**.**o**.**t             | dot      |
...
| ------ | ------------------------------------------- | --------------- | -------- | -------------- | ---------------- |
| `$0`   | 在替换字符串中包含所有匹配的字符串。        | ABC             | \[\$0\]  | x**ABC**x_AxBC | x**[ABC]**x_AxBC |
